...
interface Props {
  ...
  /** Honest live-price context (e.g. "S&P +0.4% · Nasdaq −0.2% today"); the composite itself
   *  stays the daily-close read. Shown on a trading day. */
  liveDrift?: string | null;
  /** True only during RTH — drives the "● Live" dot vs a plain "at today's close" read. */
  driftLive?: boolean;
  helpOpen: boolean;
  onToggleHelp: () => void;
...
export function RegimeCard({ regime, updatedAt, series, liveDrift, driftLive, helpOpen, onToggleHelp, help }: Props) {
  if (!regime) {
    return (
    ...
    <Card style={{ padding: '14px 16px' }}>
      ...
      {marketOpen && liveDrift && (
        <div style={{ fontSize: FONT_SIZE.xs, color: 'var(--t3)', marginTop: 8 }}>
          {driftLive && <span style={{ color: 'var(--acc)', fontWeight: FONT_WEIGHT.semibold }}>● Live </span>}{liveDrift}
        </div>
      )}
