...
import {
  useAuthStore, useUserPositions, useSyncPortfolio,
  useRiskConfig, useSectorMap, useViolationAcks, useAcknowledgeViolation,
  useIsMobile, LoadingSpinner, RegimeLight,
...
// Admin-only Limits panel (plans/integrity-guardrails.md Item 2). Flags only —
// nothing here places or blocks an order. Reads `user_positions` (the signed-in
// admin's OWN IBKR book via Flex sync) — never `holdings.current_weight` (STW's
// stated portfolio; a different book entirely, see the spec's data-domain note).
...
export function LimitsPage() {
  ...
  if (positionsLoading || configLoading) return <LoadingSpinner className="mt-16" />;

  if (!config) {
    return (
      <div className="flex-1 overflow-auto px-4 py-6">
        <div className="max-w-2xl mx-auto text-t3 text-sm">
          No risk_config row for this account yet — seed one via migration 055 or the Supabase editor.
        </div>
      </div>
    );
  }
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div>
              ...
              <div className="text-t3 text-xs max-w-md">
                Evaluates YOUR OWN IBKR book (synced via Flex Query) against the thresholds below.
                Nothing here blocks an order — breaches are flagged for you to act on.
                {config.is_placeholder && ' Thresholds are still placeholders — review before trusting them.'}
              </div>
            </div>
            <button
              onClick={sync}
