# Schema Migration Plan v3 — Multi-Leg Trade Tracking
...
- v3: addressed 2 new issues introduced by v2; signals singleton resolved as per-day history; holdings.status dropped; migration numbering cleaned up
...
duplicate `holding_transactions` rows are created.
...
- App signals read: change from single-row fetch to
  `SELECT * FROM signals WHERE trader_id = :gradoxx_id ORDER BY date DESC LIMIT 1`
...
**Expiration:** `EXPIRED_WORTHLESS` crystallizes a 100% premium loss.
Realized P&L = `-(avg_cost_basis × total_bought × multiplier)`. Size hits 0.

**Exercise:** option closes at zero realized P&L — cost basis transfers to the spawned
shares leg. Size hits 0. The routine is responsible for opening the new shares leg
with cost basis = `strike + premium_paid`.
...
**Verify loop behavior on the preview branch before production.**
Test: insert one `holding_transactions` row → confirm `holdings` updates exactly once
...
weight and thesis change audit log only.

`holding_transactions.leg` is `NOT NULL DEFAULT 1` (confirmed on live schema) —
omitting it from the insert is safe; the default applies until 035 drops the column.
...
- Add `"trader_id": "<Gradoxx uuid>"` to payload
...
**`stw-morning-run` — Gradoxx step `run_log` write:**
- Use `channel_id` pointing to the `gradoxx-vip` channel UUID
...
- Write a `holding_transactions` row with the new weight — trigger 031 propagates to `holdings.current_weight`
- `initial_weight` write-once logic now handled by the trigger — routine can pass it
  unconditionally; trigger protects it
...
**No database changes required.** `conviction_comments` already holds all rows in one
place. This is a rendering and layout change only.
...
> The `CommentaryHistory` section in the dashboard renders all `conviction_comments`
> rows for the ticker ordered by date, newest first. No archiving happens — all rows
> stay in `conviction_comments` permanently. The thesis (`holdings.summary` + `bullets`)
> is a separate explicit write, made only when the durable reason for holding the
...
| 031 | `holding_transactions` sync trigger | Requires 026 — verify loop on preview branch | Cutover |
...
3. Writers table: add `legs` and `leg_transactions` (written by routines Phase 2);
   ...
   `action_date`, `current_weight`, `initial_weight` via trigger
