...
| GEX Signals | ✅ DONE — added missing price-map intro copy ("Above the gamma-flat line…floor is gone", PR1). Kept: verdict advisory + glossary genericize dead "Graddox" name (PR2); real stale-banner data; sparkline omitted (documented); GexChart preserved. **Open Q G-GEX**: section headers use 10px uppercase emoji (🗺️/📋/📝) micro-labels vs mock's 14px/600 sentence-case titles — cosmetic + consistent across page + diverges from Macro/Settings convention; NOT auto-changed (host de-prioritizes cosmetics). Glossary reworded but accurate — kept. |
...
| Stock Picks · list | ✅ DONE — NO fixes. Filters (conviction Tier 1–6 exact, status exact, real baskets/sectors, all sorts), tier grouping (canonical "TIER N — … CONVICTION" label), badges (basket+action+Held+RegimeBadge — richer, PR1) all match. **Flag P-LIST**: row secondary shows "weight% · last-action date" vs mock's "price · weight%" — deliberate (pairs w/ status badge + Newest/Oldest sort; price on detail); not a one-source conflict. Host: want price on the row instead of date? |
...
- **G-GEX** GEX section headers render as 10px uppercase emoji micro-labels (🗺️ / 📋 / 📝) on an
  `--s2` bar, vs the mock's inline 14px/600 sentence-case titles (no emoji, no bar) — which is also the
  convention Macro & Settings follow. Consistent across the GEX page, so reads as intentional. Cosmetic;
  left as-is (host de-prioritizes cosmetics). Convert GEX to the sentence-case title convention, or keep?
